...
ChatLogic::~ChatLogic()
{
    ...
    //the delete of the chatbot is no longer manually hansdled. It is habdled by the smart pointer on the stack.
    
    /*if (_chatBot != nullptr)
    {   
        std::cout<< "ChatBot Destructor 2\n";
        delete _chatBot;
        _chatBot = nullptr;
    }*/
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        {
                            std::unique_ptr<GraphNode> n1 = std::make_unique<GraphNode>(id);
                            _nodes.emplace_back( std::move(n1));   //transfer ownership of the unique pointer to _nodes
    ...
    // create instance of chatbot
    ChatBot bot1("../images/chatbot.png");
    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    bot1.SetChatLogicHandle(this);
    // add chatbot to graph root node
    bot1.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(bot1);
    ////
    //// EOF STUDENT CODE
}
